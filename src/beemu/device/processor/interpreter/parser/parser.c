 ...
 * @param queue
 */
void emit_m1_commands(const BeemuProcessor* processor,
 ...
 */
void emit_m2_commands_for_cbxx(const BeemuProcessor* processor,
    const BeemuInstruction* instruction,
    BeemuCommandQueue* queue)
{
	const uint32_t omc = instruction->original_machine_code;
	...
	// Iterate the PC so that we can act as if the Gameboy read the instruction
	// opcode.
...
BeemuCommandQueue* beemu_parser_parse(const BeemuProcessor* processor,
    const BeemuInstruction* instruction)
{
